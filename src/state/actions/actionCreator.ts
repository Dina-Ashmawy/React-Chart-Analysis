...
let uniqueSchools: IOptionModel[] = [];
let uniqueCountry: IOptionModel[] = [];
let uniqueCamps: IOptionModel[] = [];
let allUniqueSchools: IOptionModel[] = [];
let datasets: ISelectedSchools[] = [];
let groupsBySchoolsArray: IGroupedItems[] = [];

export const getAllDataAnalysis = () => {
  return (dispatch: Action) => {
    axios.get(`https://raw.githubusercontent.com/abdelrhman-arnos/analysis-fe-challenge/master/data.json`, {}).then((res) => {
      dispatch({
...
const filterAnalysisBySelectedCountryAndCamp = (filterState: IFilterState, allAnalysis: IAnalysis[]): IGroupedItems[] => {
  ...
  if (filterState.country && filterState.camp && filterState.school) {
    filterDataByCountryAndCamp = allAnalysis.filter(
      (item: IAnalysis) => item.country === filterState.country?.value && item.camp === filterState.camp?.value
    );
...
const selectedSchoolChartData = (): ISelectedSchools[] => {
  ...
  const filterDataByCountryAndCamp: IGroupedItems[] = filterAnalysisBySelectedCountryAndCamp(filterState, allAnalysis);
...
export const getLessonsByCamp = (): ILessonsByCampData[] => {
  const lessonsSummary: ILessonsByCampData[] = [];
  const state: IState = store.getState().ChartAnalysis;
  ...
  for (let i = 0; i < groupsBySchoolsArray.length; i++) {
    ...
    const itemInSelectedSchools = datasets.find((item: ISelectedSchools) => item.label?.indexOf(schoolNameInDataSet) !== -1) ?? undefined;
...
const handleDataMonths = (itemInGroup: IAnalysis[], schoolName: string): void => {
  ...
  for (let y = 0; y < constants.months.length; y++) {
    ...
    if (monthValue.length === 0) {
      ...
      for (let selectedMonth = 0; selectedMonth < monthValue.length; selectedMonth++) {
        totalLessonsInMonth = totalLessonsInMonth + monthValue[selectedMonth].lessons;
      }
...
export const SelectedSchoolsBasedOnCountryAndCamp = (): IOptionModel[] => {
  uniqueSchools = groupsBySchoolsArray.map((item) => ({
    label: item.key,
    value: item.key
  }));
